...
def getTableData(tableID):
	...
	for region, fips in FIPS_dict.items():
		#find all geoIDs associated with fips for the table
		fips = str(fips)
		geoIDs = geoIDs_for_state(tableID, fips)
		#loop through geoIDs to add data to populate the table
		for geo in geoIDs[0:3]:
			geoID = str(geo)
			#get page for specific table ID and geoID in json
			searchlink = "http://api.censusreporter.org/1.0/data/show/latest?table_ids=" + tableID +"&geo_ids="+geoID
			data = requests.get(searchlink)
			data = data
			if not data.ok:
				break;
			data = data.json()
			geoIDkey = str(data['data'].keys()[0])
			
			tablekey = data['data'][geoIDkey].keys()[0]
			estimates = data['data'][geoIDkey][tablekey][u'estimate']
			
			#make list of tuples of (coolumnId, columnData)
			columns = []
			for colId in colIdOrder:
				cid = str(colId)
				columns.append((colId, estimates[cid]))
...
def geoIDs_for_state(tableID, fips):
	...
	geosStateData = requests.get(url)
	if not geosStateData.ok:
		return []
	geosStateData = geosStateData.json()
	keys = geosStateData.keys()
	is_key = False
	for geoKey in keys:
		...
		if geoKey == 'geography':
			is_key = True
	if is_key == True:
		geoIDs = geosStateData['geography'].keys()
	else:
		print "%s does not have data for this table" %fips
		return []
	return geoIDs
